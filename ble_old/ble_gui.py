...
- Scan named BLE devices, connect by address, subscribe to first notify characteristic (or user-specified)
- Decode 72 triplets + 1B counter → two 36-ch samples/packet; log CSV (raw only; R is not logged)
- Real-time 12×3 waveforms (FastRTPlotGrid NumPy ring buffer)
- Real-time R (ratio-of-ratios) for selected locations
- NEW: Real-time R heatmap (time × 12 locations), robust multi-λ combination (660 & 850 vs 940)
- Auto-maximize to screen; F11 toggles fullscreen

Deps: bleak, matplotlib, numpy  (Tk must be available in your Python)
"""
...
class BLEWorker(threading.Thread):
    """
    Runs an asyncio loop in a background thread to:
      - scan for devices
      - connect to selected device
      - handle notifications and push packets to a Queue
    """
    ...
    async def _connect(self, address: str, char_uuid: Optional[str]):
        ...
        try:
            ...
            # Services → choose notify characteristic
...
class CSVLogger:
    ...
    def start_data(self):
        ...
        # BIG buffer (64 KB); not line-buffered
...
class FastRTPlotGrid:
    """12×3 grid (660/940/850) with a NumPy ring buffer for fast updates."""
...
class RPlotPanel:
    """Real-time R plot for selected channels (sliding window, ~4 s shown)."""
    def __init__(self, parent, fs: float = 25.0, r_pts_window: int = 50):
        self.parent = parent
        self.fs = fs
        self.dt_per_R = 2.0 / max(1e-9, fs)  # 1 R per BLE packet (~2 raw samples)
        self.r_pts_window = int(r_pts_window)
        self.selected: List[int] = []  # 0-based [0..11]
...
# =============== NEW: Live R heatmap (time × locations) ==================
class RHeatmapPanel:
    """
    Scrolling heatmap of combined R across all 12 locations.
    - Each append_row takes a list of 12 R values.
    - Maintains a small time history; auto-scales color using robust percentiles.
    """
    def __init__(self, parent, history_rows: int = 60, cmap: str = "viridis"):
        self.parent = parent
        self.H = int(history_rows)
        self.W = 12  # 12 locations
        self.cmap = cmap
        self.buf = np.full((self.H, self.W), np.nan, dtype=np.float32)  # time × channels
        self.write_pos = 0
        self.rows_seen = 0
        self._frame = 0
        ...
        self.ax.set_yticks([])  # no explicit tick labels for time
    ...
    def _current_buffer(self) -> np.ndarray:
        """Return a view of buf ordered oldest→newest for display."""
    ...
    def redraw(self):
        ...
        # Robust auto color scale (every ~8 frames)
        if self._frame % 8 == 0:
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("BLE PPG GUI — Waveforms + R (plots & heatmap)")

        # Auto-scale to DPI and maximize to screen; fullscreen toggles
        ...
        rframe = ttk.LabelFrame(left, text="R (ratio-of-ratios) — window=100 samples (~4 s)")
        rframe.pack(fill="x", pady=(12, 4))
        self.r_vars: List[tk.IntVar] = []  # note: spacing corrected below
        self.r_vars = []
        ...
        self.r_method = tk.StringVar(value="weighted")  # 'weighted' | 'median' | 'best'
        ...
        # Selected-channel R plot
        self.rplot = RPlotPanel(right, fs=self.fs.get(), r_pts_window=50)  # ~4 s of R at 2 samples/packet
        # NEW: R heatmap (time × 12 locations)
    ...
    # ---------- Robust multi-λ R computation ----------
    def _compute_R_all12(self) -> List[float]:
        """
        Compute robust combined R for all 12 locations using last 100 raw samples:
          R660 = (AC/DC)_660 / (AC/DC)_940
          R850 = (AC/DC)_850 / (AC/DC)_940
        Combine via self.r_method: 'weighted' (default), 'median', or 'best'.
        Returns list[12] with NaN for insufficient data.
        """
    ...
    def _compute_R_for_selected(self) -> Dict[int, float]:
        """Same as before but only for user-checked locations."""
        win_n = 100
        method = (self.r_method.get() or "weighted").lower()
        eps = 1e-12
    ...
    def poll_notify_queue(self):
        ...
        # After ingesting data, update selected R plot and heatmap
        if updated:
            # Selected traces
    ...
    def redraw_plot(self):
        ...
        self.after(120, self.redraw_plot)  # ~8 FPS
